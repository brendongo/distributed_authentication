...
DefaultParams = _Params(Ed24419Group)
...
def finalize_SPAKE2PLUS(X_msg, Y_msg, K_bytes, pi_0, d):
    transcript = b"".join([sha256(pi_0).digest(),
                           X_msg, Y_msg, K_bytes, d])
...
class _SPAKE2_Base:
    ...
    def __init__(self, secret, #unused in PAKE2PLUS
        ...
        self.pi_0_scalar = params.group.password_to_scalar(self.pi_0)
...
class _SPAKE2_Asymmetric(_SPAKE2_Base):
    ...
    def _finalize(self, K_bytes, d):
        return finalize_SPAKE2PLUS(self.X_msg(), self.Y_msg(),
                                    K_bytes, self.pi_0, d)
...
class SPAKE2PLUS_A(_SPAKE2_Asymmetric):
    ...
    def __init__(self, secret, # a tuple pair
                 params=DefaultParams, entropy_f=os.urandom):
        self.pi_0, self.pi_1 = secret
        super(SPAKE2PLUS_A, self).__init__(secret, params, entropy_f)

    def finish(self, inbound_side_and_message):
        K_bytes = super(SPAKE2PLUS_A, self).finish(inbound_size_and_message)

        # PAKE 2+: Calculate additional value d
        self.pi_1_scalar = params.group.password_to_scalar(self.pi_1)
        d = self.unblinded_message.add(self.pi_1_scalar)

        key = self._finalize(K_bytes, d)
        return key 
...
class SPAKE2PLUS_B(_SPAKE2_Asymmetric):
    ...
    def __init__(self, secret, # a tuple pair
                 params=DefaultParams, entropy_f=os.urandom):
        self.pi_0, self.pi_c = secret
        super(SPAKE2PLUS_A, self).__init__(secret, params, entropy_f)

    def finish(self, inbound_side_and_message):
        K_bytes = super(SPAKE2PLUS_B, self).finish(inbound_size_and_message)

        # PAKE 2+: Calculate additional value d
        g = self.params.group
        pi_1_elem = g.bytes_to_element(self.pi_1)
        d = pi_1_elem.add(self.xy_scalar)
